using MochaMoth.SceneObject.ProgressDisplays;
...
namespace MochaMoth.SceneObject.SceneLoaders.Implementations
{
	public class InstantLoader : ISceneLoader
	{
		...
		IProgressDisplay _loadingBar = null;
		...
		IEnumerator LoadLoadingScene()
		{
			...
			//_loadingBar = Object.FindObjectOfType<IProgressDisplay>();
		}
		...
		IEnumerator UnloadActiveScene()
		{
			...
			{
				if (_loadingBar != null)
					_loadingBar.Progress(unloadingOperation.progress / 2f);
				yield return null;
			}
		}
		...
		IEnumerator LoadTargetScene(ISceneObject sceneObject)
		{
			...
			{
				if (_loadingBar != null)
					_loadingBar.Progress((loadingOperation.progress / 2f) + 0.5f);
				yield return null;
			}
		}
